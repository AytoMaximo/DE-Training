...
# Преобразовали в структурированные данные
flat_df = data_df.selectExpr(
    "data.store_name",
    "data.check_id",
    "data.timestamp",
    "explode(data.items) as item" # explode распаковывает массив items — каждая строка теперь описывает один товар в чеке.
)

# Далее добавили ts и рассчитали сумму по каждому товару
...
# Далее создали функцию для обработки новой партии за 10 секунд
