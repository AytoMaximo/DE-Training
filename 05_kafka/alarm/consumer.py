...
clickhouse.insert(
    ...
    [[
        import_info["table"],
        import_info["import_date"],
        None,  # now() установлен в колонке как значение по умолчанию
        import_info["row_count"],
        "Импорт завершён через Kafka"
    ]],
    column_names=['table_name', 'last_import_date', 'load_timestamp', 'row_count', 'comment']
)
